...
## What is LHS?
Latin hypercube sampling aims to bring the best of both worlds: the unbiased random sampling of monte carlo simulation; and the even coverage over the decision space of a grid search. It does this by ensuring values for all variables are as uncorrelated and widely varying over the range of permitted values as possible.

## Why do I need one?
The background to this is that I have been working on improving the convergence of an optimisation package at work that uses genetic algorithms. Having a good distribution of "genes", or decisions within the initial population is key to allowing a GA to effectively explore the decision space. The `ga()` function within the GA package in R allows for a _suggestions_ argument, which takes a matrix of decision values and places them within the starting population. Initially I wrote one function to create evenly spaced sequences for every decision between the lower and upper bound of allowable values, from which I enumerated all possible combinations using `expand.grid()`. I then wrote another function to take a model object and a user-defined individual count and automatically work out the nearest individual count that allowed for even sampling over the model's decision bounds.

For models with large numbers of decisions the potential combinations for even just the upper and lower bound per decision is enormous. Already with 30 independent decisions with two possible values (upper and lower bound) the number of combinations is 2^30 = 10,737,41,824, 10 times more than the number of stars in the average galaxy. Combinatorial algorithms are the worst type for growth in complexity ($$O(n!)$$). I needed a way of randomly sampling from the decisions but in a way that ensured the starting population had lots of diversity. This is the exact use case for LHS.
...
Let's assume we would like only 12 individuals from the 24 potential unique combinations, but we still want good representation of all/most possible decisions. Using the lhs library from R,
```{r}
library(lhs)
library(dplyr)
library(purrr)
